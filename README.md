...
**一个动态主题。** 为 **DeepSeek Harness (DSH) Web GUI** 做的第三方暖色皮肤：rose-pine Dawn 调的奶油底色，配上柔光、缓慢上浮的气泡与摇摆的固定星 —— 动漫、简洁、颜色干净、**会动**。

不只是换色：界面背景里有一层持续演进的装饰 —— 气泡在缓慢上浮，固定星在摇摆、缩放、明暗起伏，两侧的粒子星各自旋转并生灭。每一次眨眼，画面都略有不同。
...
这是一个**动态主题**：装饰层不是静态贴图，而是常驻运行的动画层。四组动效各自独立、互不同步：
...
| **两侧粒子星** | 各自**单向匀速**自转，并在寿命结束后于新随机位置重生 | 转 39–51 °/s，寿命 42–96s | JS 粒子引擎 + Web Animations |
...
**无障碍**：系统开启「减弱动态效果」（`prefers-reduced-motion: reduce`）时，装饰动画**全部停用** —— 固定星的动画被禁用，气泡与粒子星引擎不生成任何元素。

> 也因此，主题在**颜色之外**还有时间维度：同一套配色，静止看是一致的，动起来每次都不一样。
